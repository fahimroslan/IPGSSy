...
;// ../../fragment/yt-wall-detection/dist/shared/index.js
...
const playerErrorSelector = ".ytp-error";
...
async function handleToastNotification(node) {
    ...
    if (anchorNode[alreadyLoggedSymbol]) {
        return;
    }
    anchorNode[alreadyLoggedSymbol] = true;
...
function handleMutations(mutations) {
    for (const mutation of mutations) {
        for (const node of mutation.addedNodes) {
            ...
            const elements = [
                node,
                ...Array.from(node.querySelectorAll([adWallSelector, toastSelector, playerErrorSelector].join(","))),
            ];
            for (const elem of elements) {
                if (elem.tagName === adWallSelector.toUpperCase() && !wallDetected) {
                    ...
                    elem.matches(playerErrorSelector)) {
...
;// ../../fragment/yt-wall-detection/dist/img/check.js
...
;// ../../fragment/yt-wall-detection/dist/img/cross.js
